...
enum BDFaceDetectType {
    ...
    BDFACE_DETECT_TYPE_RGB180X320 = 2,  // rgb照片人脸检测（仅限RK356X平台，固定宽高:180x320）
    BDFACE_DETECT_TYPE_RGB320X180 = 3,  // rgb照片人脸检测（仅限RK356X平台，固定宽高:320x180）
    BDFACE_DETECT_TYPE_NIR180X320 = 4,  // nir照片人脸检测（仅限RK356X平台，固定宽高:180x320）
    BDFACE_DETECT_TYPE_NIR320X180 = 5,  // nir照片人脸检测（仅限RK356X平台，固定宽高:320x180）
};
 ...
 * @param[in]   struct BDFaceDetectConf*    conf            配置对象地址
 * @param[in]   const long                  prior_length    priorbox长度
 * @param[in]   const char*                 prior_memory    priorbox内存地址
 * @return      enum BDFaceStatusCode       状态码
 */
DLL_API enum BDFaceStatusCode bdface_load_detect(
        ...
        struct BDFaceDetectConf* conf,
        const long prior_length = 0,
        const char* prior_memory = nullptr);
 ...
 * @param[in]   BDFaceDetectConf*                   conf            配置信息
 * @return      enum BDFaceStatusCode               状态码
 */
DLL_API enum BDFaceStatusCode bdface_detect(
        ...
        struct BDFaceDetectConf* conf = nullptr);
